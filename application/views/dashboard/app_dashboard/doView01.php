...
<table class="table table-striped table-hover">
	...
	<tbody>
		...
		foreach($listDataGroup as $row):
			$jumWil =  $row->berhasil + $row->manual+ $row->proses;
		...
		<tr>
			...
			<td align="left"><a href="<?php echo base_url().$this->router->fetch_directory().$this->router->fetch_class().'/doView01Detail/'.$row->no_prop.'/'.$tgl;?>" data-hidden="siak_list" data-target="siak_edit" title="" data-placement="bottom" data-toggle="tooltip" class="ajaxify tooltips" style="text-decoration: none;"><?php echo $row->nama_prop?></a></td>
