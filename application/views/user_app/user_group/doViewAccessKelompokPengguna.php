				...
				<div class="portlet-body">
					...
					<div class="portlet-body">
						<div class="table-responsive">
							...
							<table class="table table-striped table-bordered table-hover" id="tree_access">
							   <thead>
								   <tr>
								...
								<tbody>
									...
									<tr data-tt-id="<?php echo $row->id ?>" data-tt-parent-id="<?php echo $row->parent_id ?>" data-tt-branch="<?php echo $row->jumlah_anak($data[0]->group_level)->jumlah == 0 ? "false" :"true" ?>" data-href="<?=base_url().$this->router->fetch_directory().$this->router->fetch_class().'/'.$this->router->fetch_method().'/'.$user_level.'/'.$row->id?>">
										<td><?php $data_akses=$row->menu_access($user_level);?><input type="hidden" name="akses[<?php echo $row->id?>]" value="N"><input type="checkbox" id="akses<?php echo $row->id?>" data-id="<?php echo $row->id?>" name="akses[<?php echo $row->id?>]" class="akses" value="Y" <?php if($data_akses):?>checked="checked"<?php endif;?> disabled="disabled"></td>
